...
import com.minhr.design.common_ui.view.MyPopupWindow
...
import com.minhr.design.module_mall.adapter.ADA_ChooseCategory
import com.minhr.design.module_mall.adapter.ADA_FilterPrice
import com.minhr.design.module_mall.adapter.ADA_FilterService
import com.minhr.design.module_mall.adapter.ADA_SearchGoods
...
import com.minhr.design.module_mall.dialog.DIA_Filter
import com.minhr.design.module_mall.helper.PopupFilter
import com.minhr.design.module_mall.mvp.contract.SearchContract
import com.minhr.design.module_mall.mvp.model.SearchModel
import com.minhr.design.module_mall.mvp.presenter.SearchPresenter
...
class ACT_GoodsFilter : BaseActivity<SearchPresenter, SearchModel>(), SearchContract.View {
    ...
    private val lrecyclerview by bindView<LRecyclerView>(R.id.lrecyclerview)
    private val rb_recommend by bindView<RadioButton>(R.id.rb_recommend)
    private val rb_category by bindView<RadioButton>(R.id.rb_category)
    private val rb_filter by bindView<RadioButton>(R.id.rb_filter)
    private val bg_popup by bindView<View>(R.id.bg_popup)
    private val ll_filter by bindView<ConstraintLayout>(R.id.ll_filter)
    ...
    override fun startEvents() {
        ...
        setHeaderTitle(keyword.orEmpty())
    ...
    private fun requestNet() {
        ...
        //请求选择分类和筛选弹窗的数据
        val categoryId = outCategoryId.orEmpty()
        val key = keyword.orEmpty()
        if (!TextUtils.isEmpty(categoryId)) {
            mPresenter?.getCategoryList(categoryId, "")
        } else {
            mPresenter?.getCategoryList("", key)
        }
        mPresenter?.getFilterData(categoryId)
    }
    ...
    private fun initListener() {
        ...
        tvConfirmFilter?.setOnClickListener {
            ...
            if (!TextUtils.isEmpty(selectFilterPrice)) {
                ...
                paramsMap["minPrice"] = splitArray.getOrNull(0).orEmpty()
    ...
    private fun requestGoodList(paramsMap: HashMap<String, String>) {
        ...
        val key = keyword.orEmpty()
        if (!TextUtils.isEmpty(categoryId)) {
            mPresenter?.getSearchGoods(categoryId, "", 0, sort, paramsMap)
        } else {
            mPresenter?.getSearchGoods("", key, 0, sort, paramsMap)
        }
    }
    ...
    override fun getCategoryList(dataList: List<CategoryBean>) {
        // listOf() 不是 ArrayList，强转会 ClassCastException 闪退
        val showList = ArrayList(dataList.map { it.copy(isCheck = false) })
