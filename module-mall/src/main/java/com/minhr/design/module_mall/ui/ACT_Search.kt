...
import com.minhr.design.module_mall.adapter.ADA_SearchHistory
...
import com.minhr.design.module_mall.db.AppDatabaseHelper
...
import com.minhr.design.common_ui.view.tag.TagFlowLayout
...
class ACT_Search : BaseActivity<SearchPresenter, SearchModel>(), SearchContract.View {
    private val rvHistory by bindView<RecyclerView>(R.id.rvHistory)
    private val tv_cancle by bindView<TextView>(R.id.tv_cancle)
    private val et_search by bindView<EditText>(R.id.et_search)
    private val taglayout by bindView<TagFlowLayout>(R.id.taglayout)
    private val iv_deleteAll by bindView<ImageView>(R.id.iv_deleteAll)
    ...
    private fun getLocalHistory() {
        AppDatabaseHelper.getInstance(mContext).appDataBase.historyDao().getAllHistory()
                ...
                .subscribe({ list ->
                    ...
                    Log.e("TAG", msg)
                    if (list != null) mHistoryList = ArrayList(list)
