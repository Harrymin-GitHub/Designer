        ...
        override fun intercept(chain: Interceptor.Chain): Response {
            ...
            if (headerValues != null && headerValues!!.size > 0) {
                ...
                if ("kaiyan" == headerValue) {
                    newBaseUrl = API.BASE_KAIYAN_SERVER_IP.toHttpUrlOrNull()
                } else {
...
//                        .addPathSegment("v4")//添加第二个参数
     ...
     * 建立请求(未封装Json数据的情形)
     */
    fun <T : Any> doRequest(observable: Observable<T>, observerListener: BaseObserverListener<T>): DisposableObserver<T> {
